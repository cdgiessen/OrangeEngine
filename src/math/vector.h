...
template <arithmetic T> struct Foo
{
    T x;
};
...
// min_component(vec) -> scalar
template <arithmetic T, size_t L> constexpr auto min_component(vec<T, L> const& value) noexcept
{
    T out = value[0];
...
// max_component(vec) -> vec
template <arithmetic T, size_t L> constexpr auto max_component(vec<T, L> const& value) noexcept
{
    T out = value[0];
    for (size_t i = 1; i < L; i++)
    {
